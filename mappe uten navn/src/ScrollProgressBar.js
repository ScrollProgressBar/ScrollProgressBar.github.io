(function(window) {
  ...
  // SCROLLPRGRESSBAR DEFINE-FUNCTION
  function defineScrollProgressBar() {
    ...
    function ScrollProgressBar(options) {
      ...
      for (var option in options) {
        if (init_options.hasOwnProperty(option) === false) {
          ...
          option === "placement" &&
          ["top", "left", "right", "bottom"].filter(function(pl) {
            pl === options[option];
          }).length === 0
        ) {
          throw new Error(
    ...
    ScrollProgressBar.prototype.start = function() {
      ...
      var _id = ("_scrollbar" + Math.random()).replace(".", ""); // PROBABLY A LOT OF WAYS TO DO THIS. I JUST THOUGHT THIS WAS A FUN ONE.
      _scrolledBar.id = _id;
      ...
      _scrolled =
        window.pageYOffset ||
        (_html || document.body.parentNode || document.body).scrollTop;

      // SETTING SCREENWIDTH EQUAL TO THE WIDTH OF VIEWPORT
      _screenWidth =
        window.innerWidth || _html.clientWidth || _body.clientWidth;

      // SETTING SCREENHEIGHT EQUAL TO THE HEIGHT OF VIEWPORT
      _screenHeight =
        window.innerHeight || _html.clientHeight || _body.clientHeight;
      ...
      if (
        ...
        // SETTING WIDTH IF BAR IS GOING HORIZONTAL
        css += "width: 100%;";

        // SETTING HEIGHT IF BAR IS GOING HORIZONTAL
        css += "height: " + this.options.size + ";";

        // SETTING POSITION LEFT
        css += "left: 0;";

        // SETTING TRANSFORMORIGIN TO LEFT
        ...
        // SETTING HEIGHT IF BAR IS GOING VERTICAL
        css += "height: 100%;";

        // SETTING WIDTH IF BAR IS GOING VERTICAL
        css += "width: " + this.options.size + ";";

        // SETTING POSITION TOP
        css += "top: 0;";

        // SETTING TRANSFORM-ORIGIN TO TOP
      ...
      // GIVING THE STYLESHEET A CLOSING BRACKET FOR OUR SCROLLPROGRESSBAR STYLING
      css += "}";

      // INSERTING CSS FILE WITH CORRECT STYLES INTO HEAD
      if (_styleSheet.styleSheet) {
        // This is required for IE8 and below.
        _styleSheet.styleSheet.cssText = css;
      }
        ...
        function() {
          if (!timeoutResize) {
            timeoutResize = setTimeout(function() {
              ...
              if (
                this_.options.placement === "top" ||
                this_.options.placement === "bottom"
              ) {
                _scrolledBar.style.transform = "scaleX(" + _percentage + ")";
              } else if (
                this_.options.placement === "left" ||
                this_.options.placement === "right"
              ) {
                _scrolledBar.style.transform = "scaleY(" + _percentage + ")";
              }
            }, 66);
  ...
  if (
    typeof ScrollProgressBar === "undefined" ||
    typeof ScrollProgressBar === undefined ||
    typeof ScrollProgressBar === null
  ) {
